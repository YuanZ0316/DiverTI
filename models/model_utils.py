...
from util.Plot import plot_st_embedding, plot_subtype, plot_embedding, plot_pydot_graph, plot_pseudotime, \
    plot_st_pseudotime
from util.Metrics import ClusteringMetrics


class DIPlot:
    def __init__(self, adata, pseudo_key, predict_key, di_key, emb_key, save_path=None,
        ...
        self.load_config_data(adata, pseudo_key, predict_key, di_key, emb_key, save_path,
                            start_cell_idx, root, group_frac)
        if pseudo_key not in adata.obs.columns:
            raise ValueError(f"The pseudo_key {pseudo_key} is not found in adata.obs, please verify.")
    
    def load_config_data(self, adata, pseudo_key, predict_key, di_key, emb_key,  
                         save_path=None, start_cell_idx=None, root=None, group_frac=None):
        self.adata = adata
        if pseudo_key in adata.obs_keys():
            self.pseudotime = adata.obs[pseudo_key]  
        else:
            raise Exception(f"Pseudotime key {pseudo_key} not found in adata.obs")
        ...
        if di_key in adata.uns.keys():
            self.di_matrix = adata.uns[di_key]
            self.connectivities = self.di_matrix_to_connectivity(self.di_matrix)
        else:
            raise Exception(f"DI key {di_key} not found in adata.uns")
        ...
        self.graph = self.build_di_graph(self.di_matrix)
        self.labels_map = {i: self.group_frac.loc[:, i].idxmax() for i in list(self.graph.nodes)}
    def di_matrix_to_connectivity(self, di_matrix):

        if isinstance(di_matrix, dict):
            root_key = str(self.root) if self.root else list(di_matrix.keys())[0]
            di_df = di_matrix[root_key]
        else:
            di_df = di_matrix

        n_clusters = len(di_df)
        connectivity = pd.DataFrame(
            np.zeros((n_clusters, n_clusters)),
            index=di_df.index,
            columns=di_df.columns
        )

        threshold = np.percentile(di_df.values.flatten(), 70)

        for i in range(len(di_df)):
            for j in range(len(di_df)):
                if i != j and di_df.iloc[i, j] > threshold:
                    connectivity.iloc[i, j] = 1.0
        
        print(connectivity)
        print(f"{(connectivity != 0).sum().sum()}")
        
        return connectivity
 

    def build_di_graph(self, di_matrix):
        ...
        for i in range(len(di_df)):
            for j in range(len(di_df)):
                if i != j and di_df.iloc[i, j] > threshold:
                    G.add_edge(di_df.index[i], di_df.index[j], 
                              weight=di_df.iloc[i, j],
                              di_value=di_df.iloc[i, j])
        
        return G

    def plot_di_heatmap(self, show=True):

        if isinstance(self.di_matrix, dict):
            di_df = self.di_matrix[str(self.root)] if self.root else list(self.di_matrix.values())[0]
        else:
            di_df = self.di_matrix
            
        plt.figure(figsize=(10, 8))
        sns.heatmap(di_df, annot=True, fmt=".2f", cmap="YlOrRd", 
                   cbar_kws={'label': 'Diversity Increment'})
        plt.title("Diversity Increment Matrix")
        plt.tight_layout()
        
        if self.img_path is not None:
            plt.savefig(self.img_path + 'di_heatmap.png', dpi=300)
        if show:
            plt.show()

    def plot_di_network(self, show=True):
        ...
        if hasattr(self, 'pseudotime'):
            ...
            for node in self.graph.nodes():
                cluster_pt = self.pseudotime[self.predict_labels == int(node)].mean()
                node_colors.append(cluster_pt)
        else:
            node_colors = 'lightblue'

        edge_weights = [self.graph[u][v]['di_value'] * 3 for u, v in self.graph.edges()]

        nx.draw_networkx_nodes(self.graph, pos, node_color=node_colors, 
                              node_size=800, cmap='viridis', alpha=0.9)
        nx.draw_networkx_edges(self.graph, pos, edge_color='gray', 
                              width=edge_weights, alpha=0.6, arrows=True)
        nx.draw_networkx_labels(self.graph, pos, font_size=10, font_weight='bold')

        edge_labels = {(u, v): f"{self.graph[u][v]['di_value']:.2f}" 
                      for u, v in self.graph.edges()}
        nx.draw_networkx_edge_labels(self.graph, pos, edge_labels=edge_labels, 
                                   font_size=8)
        
        plt.title("Trajectory Tree based on Diversity Increment")
        plt.axis('off')
        
        if self.img_path is not None:
            plt.savefig(self.img_path + 'di_network.png', dpi=300)
        if show:
            plt.show()
    ...
    def run_group_param(self):
        ...
        for group_i in set_labels:
            ...
            group_pt[group_idx] = np.mean(self.pseudotime.iloc[loc_i])
    ...
    def plot_st_pseudotime(self):
        plot_st_pseudotime(self.adata, self.pseudotime, save_path=self.img_path + 'st_pseudotime.png')
    ...
    def plot_pseudotime(self, show=True):
        plot_pseudotime(self.emd, self.group_frac, self.connectivities, self.predict_labels, self.pseudotime,
...
def evaluation(embedding, nclasses, labels, clu_model='kmeans'):
    ...
    return ari, ami
def get_DI_connectivities(adata, di_key, threshold_percentile=80):

    if di_key in adata.uns_keys():
        di_matrix = adata.uns[di_key]
        if isinstance(di_matrix, dict):

            di_df = list(di_matrix.values())[0]
        else:
            di_df = di_matrix
        
        di_values = di_df.values.flatten()
        di_values = di_values[di_values > 0]
        
        if len(di_values) > 0:
            threshold = np.percentile(di_values, threshold_percentile)
        else:
            threshold = 0.1
            
        connectivity = (di_df > threshold).astype(float)
        return connectivity
    else:
        raise Exception(f"DI key {di_key} not found in adata.uns")