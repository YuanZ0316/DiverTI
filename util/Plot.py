...
import seaborn as sns
import umap
def plot_pseudotime(emd, group_frac, connectivities, predict_labels, pseudotime, label_map, show=True, save_path=None,
                    colors='plasma'):
    predict_str_labels = np.array([str(i) for i in predict_labels])
    
    actual_cluster_order = [str(col) for col in group_frac.columns]
    print(f"{actual_cluster_order}")

    if not isinstance(connectivities, pd.DataFrame):
        n_clusters = group_frac.shape[1]
        default_index = [str(i) for i in range(n_clusters)]
        connectivities = pd.DataFrame(connectivities, index=default_index, columns=default_index)
    
    print(f"\n{connectivities}")
    
    non_zero_count = (connectivities != 0).sum().sum()
    print(f"{non_zero_count}")

    if non_zero_count == 0:
        print("❌ Error: connectivity matrix is empty – unable to plot trajectory!")
        print("Please verify how the connectivity matrix is generated and passed along.")
        fig, ax = plt.subplots(1, 1, figsize=(10, 10))
        ax.text(0.5, 0.5, "Error: connectivity matrix is empty\nUnable to plot trajectory",
    ...
    connectivities_clean = connectivities.astype(float).fillna(0.0)
    connectivities_clean.columns = connectivities_clean.index.tolist()
    
    graph = nx.from_pandas_adjacency(connectivities_clean, create_using=nx.DiGraph)
    print(f"{list(graph.edges())}")
    print(f"{list(graph.nodes())}")
    
    def get_centriods(emb, pseudotime, group_frac):
        center_pos = pd.DataFrame(
            np.zeros([group_frac.shape[1], 2]), 
            columns=['x', 'y'], 
            index=[str(col) for col in group_frac.columns]
        )
        center_pseudotime = pd.DataFrame(
            np.zeros([group_frac.shape[1], 1]), 
            columns=['pseudotime'],
            index=[str(col) for col in group_frac.columns]
        )
        center_size = pd.DataFrame(
            np.zeros([group_frac.shape[1], 1]), 
            columns=['size'],
            index=[str(col) for col in group_frac.columns]
        )
        
        for i in center_pos.index:
            idx = np.where(predict_str_labels == i)[0]
            if len(idx) > 0:
                center_pos.loc[i, :] = np.mean(emb[idx, :], axis=0)
                center_pseudotime.loc[i, 'pseudotime'] = np.mean(pseudotime.iloc[idx])
                center_size.loc[i, 'size'] = len(idx)
        return center_pos, center_pseudotime, center_size

    fig, ax = plt.subplots(1, 1, figsize=(10, 10))
    reducer = umap.UMAP(n_components=2, random_state=5, n_jobs=1)
    emd_2pc = reducer.fit_transform(emd)
    centroids, center_pseudotime, center_size = get_centriods(emd_2pc, pseudotime, group_frac)
    
    data_min, data_max = min(pseudotime), max(pseudotime)
    norm = Normalize(vmin=data_min, vmax=data_max)
    sm = ScalarMappable(cmap=plt.get_cmap(colors), norm=norm)

    sns.scatterplot(
        x=emd_2pc[:, 0], y=emd_2pc[:, 1], 
        c=[sm.to_rgba(i) for i in pseudotime], 
        s=15, ax=ax, legend=False
    )

    node_pos = {}
    for node in graph.nodes():
        if node in centroids.index:
            node_pos[node] = centroids.loc[node, :].values
    
    nx.draw_networkx_nodes(
        graph, pos=node_pos,
        node_size=[center_size.loc[node, 'size'] * 10 for node in graph.nodes()],
        node_color=[sm.to_rgba(center_pseudotime.loc[node, 'pseudotime']) for node in graph.nodes()],
        ax=ax, alpha=0.8
    )
    ...
    nx.draw_networkx_labels(
        graph, pos=node_pos,
        labels=label_map, ax=ax, font_size=25
    )
...
def plot_st_embedding(adata, predict_labels, colors='tab10', save_path=None, show_trajectory=False, group_frac=None,
    ...
    if isinstance(colors, str):
        ...
        color = [colors[int(i)] for i in predict_labels]
    else:
        color = [colors[i] for i in predict_labels]
    ax1.scatter(locations[:, 0], locations[:, 1], c=color, s=20)
    ax1.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=12, frameon=False)
...
def plot_st_pseudotime(adata, pseudotime, save_path=None):
    # locations = adata.obsm['spatial'][:, [1, 0]]
    locations = adata.obsm['spatial'][:, [0, 1]]
    # locations[:, 0] = -locations[:, 0]
    locations[:, 1] = -locations[:, 1]
    ...
    ax1.scatter(locations[:, 0], locations[:, 1], c=[sm.to_rgba(i) for i in pseudotime], s=20)
